...
\begin{abstract}
This paper introduces an Energy-layer calibration procedure for commit/defer
decision support using existing paired datasets anchored by two upstream
inputs: a fixed NRR-Patterns benchmark with a selected Stage B
boundary-honesty subset, and a fixed 2026-03-07 Boundary-derived snapshot that
contains the Stage B and ordered-combo paired files used here. The method
fixes utility construction and calibration-only thresholding before evaluation.
Utility is defined from quality-improvement and rework-cost terms, and two cost
formulations are compared: gross rework improvement and output-overhead-
adjusted net cost. Under tested conditions, net-cost calibration expands
decision coverage relative to gross-cost calibration, but the quality of that
expansion is condition-sensitive. In Stage B, net-cost calibration increases
coverage while also increasing disagreement against calibration-derived oracle
labels; in ordered-combo data, net-cost calibration increases coverage without
increasing overall disagreement. Sensitivity analyses identify disagreement-
prone regions concentrated in Gemini and in Stage B at higher temperature,
while complementary sign-flip summaries remain heterogeneity warnings rather
than a main result. Narrow forward checks also replay exactly on the tested
covered slices while exposing a provider-specific transfer boundary. Anthropic
remains route-stable on the tested branch and branch-adjacent combo stable
slices, whereas Gemini already shows stable-slice route instability on the
tested branch covered slice and that instability sharpens on the tested combo
opening. Because realized forward outcomes remain matched between baseline and
Energy arms in these batches, these forward checks support boundary mapping
rather than gain claims. Taken together, the results define a reproducible
calibration procedure and clarify where direct policy claims still require
additional forward paired validation.
\end{abstract}
...
NRR-Energy is one module in the NRR research program:
Core $\rightarrow$ Phi $\rightarrow$ IME $\rightarrow$ Transfer $\rightarrow$ Coupled $\rightarrow$ NRR-Patterns $\rightarrow$ Energy.
Core and Phi established state- and entropy-side operator behavior~\cite{saito2025nrr,saito2026phi}. IME and Transfer established extraction reliability and transfer behavior~\cite{saito2026ime,saito2026transfer}. Coupled provides a dependency-propagation check cited here for context rather than as a direct evidentiary input to the present Energy calibration claims~\cite{saito2026coupled}. NRR-Patterns provides the upstream delayed-commitment family benchmark together with the selected Stage B boundary-honesty layer reused here~\cite{saito2026principles}. Separate from that benchmark, the paired-input files used here come from the fixed 2026-03-07 Boundary-derived snapshot recorded in the reproducibility note. Energy uses these frozen inputs to calibrate a commit/defer utility instrument, to report disagreement-prone regions, and to disclose complementary sign-flip and non-effective context in the paired inputs.

This paper is a calibration study. It asks two main questions under fixed
procedures: (i) can an Energy utility instrument be defined and reproduced, and
(ii) under which conditions does that calibrated instrument produce stable
versus unstable decision behavior. It also reports a narrow forward-transfer
boundary: whether a frozen routed Stage A path and the first covered forward
slices can execute and replay exactly without upgrading the evidence to a
policy-improvement claim. It does not answer whether an online Energy policy
improves outcomes prospectively versus baseline under new paired runs.
Downstream \texttt{Policy-Verification} materials are reserved for later work.
...
\begin{enumerate}
  \item A reproducible calibration procedure for an Energy-style commit/defer utility instrument under existing paired inputs anchored by a fixed NRR-Patterns benchmark together with a separate Boundary-derived input snapshot.
  \item Multi-profile calibration reporting across quality- and cost-weighted utility settings.
  \item Explicit boundary reporting for calibration disagreement regions and implementation-gated forward transfer boundaries, together with complementary sign-flip/non-effective context, including provider, temperature, and ordered-combination effects.
  \item A clear separation between procedure-level calibration claims, split-specific findings, and forward transfer boundaries.
\end{enumerate}
...
\begin{itemize}
  \item Stage B paired metrics (\texttt{stats/stageb\_all/mst\_pair\_metrics.csv}), which overlap the fixed upstream benchmark and the frozen Boundary-derived input snapshot
  \item ordered-combo paired metrics (\texttt{stats/combo\_rep3\_all/mst\_pair\_metrics.csv}), reused from the same frozen Boundary-derived input snapshot as a calibration extension
  \item raw run traces for output-token and total-overhead reconstruction in the cost analysis
\end{itemize}

The present split uses calibration rows from reps 1 and 2 and test rows from
rep 3. This split instance is provisional; the split procedure itself is fixed.
...
the \path{misinterpretation_rate} and \path{rework_cost_tokens} entries and
the row must satisfy \path{tau_trace_pass = True}.

We then define a quality term and three cost terms directly from the shipped
artifacts:
\begin{itemize}
  \item $Q =$ the \path{improvement} value from the
  \path{misinterpretation_rate} row in \path{mst_pair_metrics.csv}
  \item $C_{\mathrm{gross}} =$ the \path{improvement} value from the
  \path{rework_cost_tokens} row in \path{mst_pair_metrics.csv}
...
No separate noise-floor decision branch is carried in the current frozen
benchmark. The current freeze retains only this quantile-threshold route.
...
E1-07 & Not carried into the current frozen benchmark; historical worksheet diagnostic branch only. & Sec.~\ref{sec:profiles} \\
...
Downstream \texttt{Policy-Verification} notes are retained for later
validation work. They are not part of the core manuscript evidence claims that
this paper closes.
...
The first branch-adjacent two-direction combo opening sharpened that contrast.
Shipped forward summary reports show that Anthropic replayed exactly on
\(48/48\) pairs and kept the stable slice pattern-side for both tested
directions (\(16/16\) total), whereas Gemini also replayed exactly on
\(48/48\) pairs but sent the same stable combo slice fully baseline-side
(\(16/16\)). In all of these forward batches, realized baseline and Energy
outcomes remained matched within batch. The forward evidence is therefore
operational and boundary-facing at the summary-report level: it shows that the
current instrument can be executed on the tested slices and that the resulting
read delimits a provider-specific transfer boundary, while also preventing any
gain claim at this stage.
...
The disagreement table above should not be read as the only boundary type in
the current Energy line. Complementary sign-flip boundary summaries in the
paired inputs disclose provider-reversal patterns together with zero-provider
support columns as a non-effective-context layer. In the current paper we
therefore keep calibration disagreement as the main split-level boundary read,
while treating the sign-flip/non-effective layer as paired-input heterogeneity
disclosure rather than a fully tabulated manuscript result. The former is a
property of the current split-level decision rule under the shipped
calibration procedure; the latter remains a paired-input heterogeneity warning
that constrains how broadly any calibration-side read should be generalized.
...
The same $\Delta U$ framing can also be read as a bounded cost-of-resolution
instrument. Under this read, a commit-side move corresponds to executing
deferred comparative resolution at the current output boundary, whereas a
defer-side move keeps unresolved alternatives active and postpones that
resolution step. A positive $\Delta U$ therefore means not that cost
disappears, but that under the tested condition the delayed-resolution route
has higher net utility than immediate collapse at that step. The
disagreement-prone regions reported here, together with the sign-flip and
non-effective summaries, mark conditions where that delay no longer yields a
stable net advantage. This remains a local interpretation of the present
calibration signal, not a general complexity claim.
...
A third boundary concerns the routed Stage A implementation path that this
calibration is intended to support later. The live implementation-trial
surface records extractor identity, freeze status, and calibrated \(q/c\)
moments for the four routed runs, but it does not isolate subterm
contributions inside the routed shared-prefix readout. Under the present live
checks, that readout is therefore best treated as a bounded response-quality
calibration signal rather than as a direct grounded-state-quality measure.
...
A fifth boundary is scope. Downstream \texttt{Policy-Verification} materials
are reserved for later validation work, not treated as closed Energy
paper-level evidence claims. They are useful for tracing how the
calibration/control layer connects to later operational checks, but they do not
widen the present paper into a downstream policy-superiority or later-horizon
closure result.

\section{Limitations}
\label{sec:limitations}

The present evaluation remains a calibration-side internal consistency check
rather than a prospective paired policy test. The current oracle labels are
derived from calibration-split mean $\Delta U$ signs, so the evidence supports
reproducible calibration and boundary reporting under the tested setup but does
not by itself authorize online policy-improvement claims.

The implementation-gated forward check is also intentionally narrow. The
Stage A path is operational on the tested covered slices, but the
routed shared-prefix readout is not treated here as a direct grounded-state-quality
measure, and the forward evidence is limited here to the summary
reports rather than a full raw rerun or audit export.

Downstream \texttt{Policy-Verification} materials remain reserved for later
validation work. They trace how the Energy calibration/control layer connects
to later operational checks, but they are not part of the core Energy
paper-level evidence claim closed in this manuscript.

\section{Conclusion}
\label{sec:conclusion}

NRR-Energy contributes a calibrated commit/defer utility instrument under
existing paired inputs anchored by a fixed NRR-Patterns benchmark and a
separate frozen Boundary-derived input snapshot, together with a narrow
forward-boundary read on how that instrument transfers to frozen routed
execution. The current evidence supports reproducible calibration and boundary
reporting, not prospective policy-superiority claims. Under tested conditions,
net-cost calibration expands callable coverage relative to gross-cost
calibration, but the quality of that expansion depends on provider,
temperature, and ordered-combination cell. Gemini-centered instability, Stage
B at higher temperature, and specific Combo directions remain explicit
disagreement-prone regions, while the forward check adds a provider-specific
transfer boundary: Gemini already shows stable-slice route instability on the
tested branch covered slice, and the tested combo opening sharpens that
boundary further.

The practical contribution is therefore condition-bounded instrumentation: a
way to measure when a commit/defer signal is more or less trustworthy before
promoting it to a live policy rule. The paper closes at procedure-level
calibration claims, split-bounded findings, and forward transfer-boundary
reporting. Downstream validation remains for later work.

\section{Reproducibility}
\label{sec:repro}

Artifacts retained for this paper are:
\begin{itemize}
...
The Stage A implementation-trial summary is supported by preserved
decision-context indices, condition-metadata subsets, and routed-run audit
artifacts for the four covered runs. The three downstream comparison memos are
paired with their provider-level run-annotation CSVs: Anthropic/Gemini baseline
and Energy-policy annotations for the priority-resolution and downstream-
boundary slices, and Anthropic/Gemini baseline and revised-Energy-policy
annotations for the repaired side-topic/quarantine slice.

The vendored calibration inputs retained for this paper are stored under:
\begin{itemize}
...
Later forward boundary evidence remains at the summary-report level only. Some
historical forward raw payload, bundle, and decision-context files referenced
by the later forward summaries are not retained here, so the exact-replay
counts reported above are supported at the summary-report level rather than by
a full raw rerun.

These inputs function here as frozen paired inputs for Energy calibration.
Their physical origin is the fixed 2026-03-07 Boundary-derived snapshot. The
fixed NRR-Patterns line anchors the upstream family benchmark plus the selected
Stage B boundary-honesty read, while ordered-combo reuse serves as a
calibration extension from the same Boundary-derived snapshot. Detailed source
and copy provenance are recorded in the reproducibility note rather than
repeated here. The upstream reference artifact is
\path{paper7_integrated_manuscript_v0_29_2026-04-01.tex} with companion
checksum manifest \path{paper7_integrated_checksums_sha256.txt}, as recorded in
\path{results/analysis/energy_upstream_paper7_fixed_reference_note_v2_2026-04-01.md}.

\section*{Acknowledgments}

The author acknowledges language refinement, formatting support, and
proofreading assistance from Claude (Anthropic), ChatGPT/Codex (OpenAI), and
Gemini (Google). All claims, judgments, and final decisions in this manuscript
...
\newblock Unpublished manuscript, 2026.
\url{https://github.com/kei-saito-research/hidden-state-interface-reuse}.

\bibitem{saito2026coupled}
K.~Saito,
\newblock ``NRR-Coupled (cp): Dependency-Consistency Evaluation for Coupled State Updates,''
\newblock Unpublished manuscript, 2026.
\url{https://github.com/kei-saito-research/nrr-coupled}.
