...
The implementation-gated forward surface is also intentionally narrow. The
shipped Stage A path is operational on the tested covered slices, but the
routed shared-prefix readout is not treated here as a direct grounded-state-quality
measure, and the forward package closes at the shipped summary-memo surface
rather than at a full raw rerun or audit export.

Downstream `Policy-Verification` materials remain bounded handoff references for
later validation work. They trace how the Energy calibration/control layer
connects to later operational checks, but they are not part of the core Energy
...
Current manuscript-side artifacts on the repository current package surface are:
\begin{itemize}
...
On the repository current package surface, the shipped Stage A implementation-trial summary memo is accompanied by its
...
The repository current package surface ships the Stage A implementation-trial summary
together with its directly linked local support artifacts. The review-drop
handoff used for external review may contain only the current manuscript source,
while the repository current package retains the broader reproducibility layer.
Later forward
...
forward summary memos are not retained inside the repository current package, so
...
The fixed upstream authority artifact for this handoff is
\path{paper7_integrated_manuscript_v0_29_2026-04-01.tex} with companion checksum
manifest \path{paper7_integrated_checksums_sha256.txt}, as recorded in
