...
MAIN_TEX="$(find "$CURRENT_DIR" -maxdepth 1 -type f -name 'nrr-energy_manuscript_v*.tex' | sort -V | tail -n 1)"
OUT_DIR="${1:-/tmp/nrr-energy_current_build}"
...
if command -v tectonic >/dev/null 2>&1; then
  ...
  echo "Neither tectonic nor latexmk is available." >&2
  exit 1
fi
