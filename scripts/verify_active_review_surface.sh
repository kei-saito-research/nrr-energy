...
while IFS= read -r file; do
  tex_files+=("$file")
done < <(find "$CURRENT_DIR" -maxdepth 1 -type f -name 'nrr-energy_manuscript_v*.tex' | LC_ALL=C sort)

pdf_files=()
while IFS= read -r file; do
  pdf_files+=("$file")
done < <(find "$CURRENT_DIR" -maxdepth 1 -type f -name 'nrr-energy_manuscript_v*.pdf' | LC_ALL=C sort)
